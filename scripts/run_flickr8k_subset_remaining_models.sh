...
RUN_RESNET_GMLP="${RUN_RESNET_GMLP:-1}"
RUN_CLIP_CROSS_ATTN_GATE="${RUN_CLIP_CROSS_ATTN_GATE:-1}"
RUN_CLIP_ATTN_PREFIX="${RUN_CLIP_ATTN_PREFIX:-1}"
RUN_CLIP_GATED_SEQ_CONCAT="${RUN_CLIP_GATED_SEQ_CONCAT:-1}"
...
SUMMARY_TSV="${SUMMARY_TSV:-$SUITE_ROOT/summary_remaining.tsv}"
...
# Re-train the unfinished subset baseline first when requested, then fan out the CLIP runs.
if [[ "$RUN_RESNET_GMLP" == "1" ]]; then
  run_one_combo resnet_gmlp concat_temp 18
else
  echo "[SKIP] encoder=resnet_gmlp fuser=concat_temp run_resnet_gmlp=0"
fi

selected_clip_jobs=()
if [[ "$RUN_CLIP_CROSS_ATTN_GATE" == "1" ]]; then
  selected_clip_jobs+=("clip|cross_attn_gate|18")
else
  echo "[SKIP] encoder=clip fuser=cross_attn_gate run_clip_cross_attn_gate=0"
fi
if [[ "$RUN_CLIP_ATTN_PREFIX" == "1" ]]; then
  selected_clip_jobs+=("clip|attn_prefix|18")
else
  echo "[SKIP] encoder=clip fuser=attn_prefix run_clip_attn_prefix=0"
fi
if [[ "$RUN_CLIP_GATED_SEQ_CONCAT" == "1" ]]; then
  selected_clip_jobs+=("clip|gated_seq_concat|18")
else
  echo "[SKIP] encoder=clip fuser=gated_seq_concat run_clip_gated_seq_concat=0"
fi

if [[ "${#selected_clip_jobs[@]}" -eq 0 ]]; then
  echo "[SKIP] no clip jobs selected"
elif [[ -n "$CLIP_PARALLEL_GPUS" ]]; then
  IFS=',' read -r -a clip_gpus <<< "$CLIP_PARALLEL_GPUS"
  if [[ "${#clip_gpus[@]}" -lt "${#selected_clip_jobs[@]}" ]]; then
    echo "[ERROR] CLIP_PARALLEL_GPUS requires at least ${#selected_clip_jobs[@]} comma-separated GPU ids, got: $CLIP_PARALLEL_GPUS" >&2
    exit 1
  fi
  clip_pids=()
  for idx in "${!selected_clip_jobs[@]}"; do
    IFS='|' read -r encoder fuser resnet_depth <<< "${selected_clip_jobs[$idx]}"
    run_one_combo_on_gpu "${clip_gpus[$idx]}" "$encoder" "$fuser" "$resnet_depth"
  done
  clip_failed=0
  ...
  for clip_job in "${selected_clip_jobs[@]}"; do
    IFS='|' read -r encoder fuser resnet_depth <<< "$clip_job"
    run_one_combo "$encoder" "$fuser" "$resnet_depth"
  done
fi
