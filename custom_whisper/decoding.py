...
class DecodingResult:
    ...
    nbest_texts: List[str] = field(default_factory=list)
    nbest_avg_logprobs: List[float] = field(default_factory=list)
    nbest_tokens: List[List[int]] = field(default_factory=list)
...
class PyTorchInference(Inference):
    ...
    def logits(self, tokens: Tensor, audio_features: Tensor) -> Tensor:
        ...
        return self.model.decoder(
            ...
            prefix_insert_pos=prefix_insert_pos,
        )
...
class DecodingTask:
    ...
    def __init__(self, model: "Whisper", options: DecodingOptions):
        ...
        self.n_ctx: int = model.dims.n_text_ctx - soft_prefix_len
        if self.n_ctx <= 0:
            raise ValueError(
                f"decoder_prompt_len={soft_prefix_len} leaves no text context in "
                f"n_text_ctx={model.dims.n_text_ctx}"
            )
    ...
    def run(self, mel: Tensor) -> List[DecodingResult]:
        ...
        all_token_lists: List[List[List[int]]] = [
            [candidate.tolist() for candidate in group] for group in tokens
        ]
        all_texts: List[List[str]] = [
            [tokenizer.decode(candidate).strip() for candidate in group]
            for group in all_token_lists
        ]
        all_avg_logprobs: List[List[float]] = [
            [score / (len(candidate) + 1) for candidate, score in zip(group_tokens, group_scores)]
            for group_tokens, group_scores in zip(all_token_lists, sum_logprobs)
        ]

        # select the top-ranked sample in each group
        selected = self.sequence_ranker.rank(tokens, sum_logprobs)
        tokens: List[List[int]] = [t[i] for i, t in zip(selected, all_token_lists)]
        texts: List[str] = [t[i] for i, t in zip(selected, all_texts)]
        ...
        return [
            DecodingResult(
                ...
                nbest_avg_logprobs=nbest_scores,
                nbest_tokens=nbest_tokens,
            )
            for text, language, tokens, features, avg_logprob, no_speech_prob, nbest_texts, nbest_scores, nbest_tokens in zip(
                *fields
            )
        ]
